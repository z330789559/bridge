...
async function scanBlock(opts, api, moduleMetadata, admin, web3, contract, blockNum) {
    ...
    for (let event of events) {
        ...
        switch (event.event) {
            case 'Transfer':
                // TODO: check the existence of event.transactionHash in parami before send below tx to parami.
                if (event.returnValues.to === opts.ethHotWallet) {
...
async function scan(opts, from_block) {
    ...
    for (; ;) {
        try {
            ...
            if (from_block === 0) from_block = bestBlockNum - opts.depth;
